...
let passports = rawPassports.split(/\n\n/).map(passportStr => {
    let passport = {};
    passportStr.split(/[\n ]+/).map(str => {
...
let validCount = 0;
passports.forEach(passport => {
    if (!/^\d{0,4}$/.test(passport.byr)) {
        console.log("invalid byr", passport.byr);
        return;
    }
    let byr = parseInt(passport.byr, 10);
    if (isNaN(byr) || !(passport.byr >= 1920 && passport.byr <= 2002)) {
        console.log("invalid byr 1920-2002", byr);
        return;
    }

    if (!/^\d{0,4}$/.test(passport.iyr)) {
        console.log("invalid iyr", passport.iyr);
        return;
    }
    let iyr = parseInt(passport.iyr, 10);
    if (isNaN(iyr) || !(passport.iyr >= 2010 && passport.iyr <= 2020)) {
        console.log("invalid iyr 2010-2020", iyr);
        return;
    }

    if (!/^\d{0,4}$/.test(passport.eyr)) {
        console.log("invalid eyr", passport.eyr);
        return;
    }
    let eyr = parseInt(passport.eyr, 10);
    if (isNaN(eyr) || !(passport.eyr >= 2020 && passport.byr <= 2030)) {
        console.log("invalid eyr 2020-2030", eyr);
        return;
    }
    ...
    if (passport.hgt.endsWith("cm")) {
        if (isNaN(hgt) || !(hgt >= 150 && hgt <= 193)) {
            console.log("invalid hgt cm 150-193", hgt, passport.hgt);
            return;
        }
    } else {
        if (isNaN(hgt) || !(hgt >= 59 && hgt <= 76)) {
            console.log("invalid hgt in 59-76", hgt, passport.hgt);
            return;
        }
    }

    if (!/^#[0-f]{0,6}$/.test(passport.hcl)) {
        console.log("invalid hcl", passport.hcl);
        return;
    }

    if (!"amb blu brn gry grn hzl oth".split(" ").includes(passport.ecl)) {
        console.log("invalid ecl 'amb blu brn gry grn hzl oth'", passport.ecl);
        return;
    }

    if (!/^\d{9}$/.test(passport.pid)) {
        console.log("invalid pid", passport.pid && passport.pid.length, passport.pid);
        return;
    }
    validCount++;
});

console.log(validCount); // off by one???
