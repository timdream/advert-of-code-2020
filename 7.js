let input = `light gold bags contain 2 light lime bags, 1 faded green bag, 3 clear olive bags, 2 dim bronze bags.
...
clear brown bags contain no other bags.`.split("\n");

let bags = {};
input.forEach(line => {
    let [ name, contentStr ] = line.split(" bags contain ");
    let content = contentStr.split(", ").map(contentName => {
        contentName = contentName.replace(/ bags?.?$/, "");
        let count = parseInt(contentName, 10);
        return [count, contentName.replace(/^\d+ /, "")];
    });

    if (content.length === 1 && content[0][1] === "no other") {
        content = [];
    }
...
console.dir(bags, { depth: null });

for ([name, content] of Object.entries(bags)) {
    content.forEach(([n, contentName], i) => {
        content[i] = [n, bags[contentName]];
    });
}

function canContainShinyGoldBag(content) {
    let canContain = false;
    for (let i = 0; i < content.length; i++) {
        let [n, subContent] = content[i];
        if (subContent === bags["shiny gold"]) {
            canContain = true;
            break;
        }
        if (canContainShinyGoldBag(subContent)) {
            canContain = true;
            break;
        }
    }
...
let count = 0;
for ([name, content] of Object.entries(bags)) {
    if (canContainShinyGoldBag(content)) {
        count++;
    }
}
...
function totalBagCount(content) {
    let total = 1; // Count itself
    for (let i = 0; i < content.length; i++) {
        let [n, subContent] = content[i];
        total += n * totalBagCount(subContent);
    }
    return total;
}

console.log(totalBagCount(bags["shiny gold"]) - 1); // Discount the shiny gold bag
